...
if (empty($authHeader)) {
    ...
    exit(); // <-- PROGRAM BERHENTI DI SINI, POST/GET DIBATALKAN
}
...
// handle POST req dri ESP
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['nilai_asap'])) {
        // real_escape_string ini adalah pencegah SQL Injection!
...
// handle GET request dari React
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    header("Content-Type: application/json");

    $data_sensor = [];

    // Mengambil 500 data terbaru agar filter Dashboard React berfungsi maksimal
    $sql = "SELECT * FROM log_sensor ORDER BY id DESC LIMIT 500";
